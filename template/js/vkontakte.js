// VKONTAKTE.RU API JavaScript for creating group badges
...
if (!VK.Widgets) {
  ...
  VK.Widgets._constructor = function(widgetUrl, objId, options, params, funcs, defaults) {
    ...
    width = width ? (Math.max(defaults.minWidth || 134, Math.min(10000, width)) + 'px') : '100%';
    if (!params.url) params.url = options.pageUrl || location.href.replace(/#.*$/, '');
    var url = base_domain + '/' + widgetUrl + '?app=' + (VK._apiId || '0') + '&width=' + width;
    for (i in params) {
      if (i == 'title' && params[i].length > 40) params[i] = params[i].substr(0, 40)+'...';
      if (i == 'description' && params[i].length > 100) params[i] = params[i].substr(0, 100)+'...';
